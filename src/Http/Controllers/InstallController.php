...
namespace iteclab\itecCreator\Http\Controllers;
...
use iteclab\itecCreator\Events\EnvironmentSaved;
use Illuminate\Support\Facades\Http;
use Illuminate\Support\Facades\Log;
use iteclab\itecCreator\Http\Helpers\DatabaseManager;
...
class InstallController extends Controller
{
    ...
    public function preInstall()
    {
        ...
        return view('iteclab.creator.pre-install', compact('route_value', 'resource_value', 'public_value', 'storage_value', 'env_value'));
    }
    ...
    public function configuration()
    {
        ...
        if (session()->has('validated')) {
            $data['is_active'] = true;
            return view('iteclab.creator.config', $data);
        }
        return redirect(route('itecCreator::pre-install'));
    }
    ...
    public function serverValidation(Request $request)
    {
        if ($this->phpversion() > 7.0 && $this->mysqli() == 1 && $this->curl_version() == 1 && $this->allow_url_fopen() == 1 && $this->openssl() == 1 && $this->pdo() == 1 && $this->bcmath() == 1 && $this->ctype() == 1 && $this->fileinfo() == 1 && $this->mbstring() == 1 && $this->tokenizer() == 1 && $this->xml() == 1 && $this->json() == 1) {
            ...
            return redirect(route('itecCreator::config'));
        }
            ...
            if (true) {
                if (true) {
                    try {
                        ...
                        return Redirect::route('itecCreator::database')->with(['results' => $results]);
                    } catch (\Exception $e) {
